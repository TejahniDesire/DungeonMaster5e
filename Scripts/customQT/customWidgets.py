...
from . import (style, customWindows, QTHelper)
...
class RPWidget:
    ...
    def __init__(self, top_stats: list[charecterAttributes.TopStatValue],time):
        ...
        left_layout.addLayout(upper_layout)
    ...
    def alignment_window(self):
        self.alignment_w = customWindows.AlignmentWindow(self.tp['alignment'].set, self.update)
...
class WeaponWidget:
    ...
    def __init__(self,tinventory:inventory.Inventory,attack_inventory:inventory.AttackInventory,ability_score_counter:dict[charecterAttributes.Stat]):
    ...
    def refresh(self):
        QTHelper.deleteItemsOfLayout(self.mobile_layout)
        self.mobile_layout = QVBoxLayout()
        self.main_layout.addLayout(self.mobile_layout)
        self.defaultStateCreation()
    ...
    def defaultStateCreation(self):
        ...
        self.mobile_layout.addWidget(QTHelper.CreateLabel("Attacks and SpellCasting", style.LabelFont2),
                                   alignment=QtCore.Qt.AlignCenter)

        # Grid Layout
        grid_layout = QGridLayout()
        # grid_layout.SetMaximuHeight
        grid_layout.addWidget(QTHelper.CreateLabel("Name", style.LabelFont2, style.DarkGreyLabel),
                              0, 1, alignment=QtCore.Qt.AlignCenter)
        grid_layout.addWidget(QTHelper.CreateLabel("Attack Bonus", style.LabelFont2, style.DarkGreyLabel),
                              0, 3, alignment=QtCore.Qt.AlignCenter)
        grid_layout.addWidget(QTHelper.CreateLabel("Damage/Type", style.LabelFont2, style.DarkGreyLabel),
                              0, 5, 1, 2, alignment=QtCore.Qt.AlignCenter)

        add_button = QTHelper.CreateGenButton(stylesheet=style.ItemEditButton,
        ...
        for i in range(8):
            grid_layout.addWidget(QTHelper.CreateSeperator(), 1, i)
        ...
        for key in self.ATiventory.inventory.keys():
            ...
            grid_layout.addWidget(QTHelper.CreateVSeperator(), counter, 2)

            grid_layout.addWidget(bonus, counter, 3, alignment=QtCore.Qt.AlignCenter)
            grid_layout.addWidget(QTHelper.CreateVSeperator(), counter, 4)

            grid_layout.addWidget(damage, counter, 5, alignment=QtCore.Qt.AlignCenter)
            grid_layout.addWidget(QTHelper.CreateVSeperator(), counter, 6)

            grid_layout.addWidget(subtract_button, counter, 7, alignment=QtCore.Qt.AlignCenter)
            grid_layout.addWidget(QTHelper.CreateVSeperator(), counter, 6)

            for i in range(8):
                grid_layout.addWidget(QTHelper.CreateSeperator(), counter+1, i)
            counter += 2

        self.mobile_layout.addLayout(grid_layout)

    def createWeaponRow(self,item:itemsDnD.Item):
        name = str(item.get_amount()) + "x " +  item.get_name()
        name_label = QTHelper.CreateLabel(name,style.LabelFont2p5,style.Attacklabel)
        name_label.setMinimumWidth(10)

        bonus_value = self.ABS[item.getScaling()].get_total_bonus()

        sign = pyHelper.sign_string(bonus_value)
        bonus = sign + str(bonus_value)
        bonus_label = QTHelper.CreateLabel(bonus,style.LabelFont2p5,style.Attacklabel)

        damage = str(item["damage_dice"]) + " " + str(item["damage_type"])
        damage_label = QTHelper.CreateLabel(damage,style.LabelFont2p5,style.Attacklabel)

        subtract_button = QTHelper.CreateGenButton(stylesheet=style.ItemEditButton,
    ...
    def addWindow(self):
        self.add_w = customWindows.AddWindow(self.tinventory,self.ATiventory, self.update)
        self.add_w.show()
...
class RPTraitWidget:
    ...
    def __init__(self, rp_traits):
        ...
        main_layout.addWidget(QTHelper.CreateSeperator())
        main_layout.addWidget(QTHelper.CreateSeperator())
        main_layout.addWidget(frame1)
        main_layout.addWidget(frame2)
...
class InventoryWidget:
    ...
    def __init__(self,tinventory:inventory.Inventory):
        self.main_widget = QFrame()
        ...
        self.widgets = {}
        for i in range(5):
            money_layout.addWidget(QTHelper.CreateSeperator(), 0, i)
        ...
        for i in range(1,2*len(money_text)+1,2):
            ...
            money_layout.addWidget(QTHelper.CreateLabel(money_text[int(i / 2)], style.LabelFont1, money_style), i, 1)
            money_layout.addWidget(QTHelper.CreateSeperator(), i + 1, 1)

            # CURRENCY AMOUNT LABELS
            self.widgets["currency_amounts"] += [QTHelper.CreateLabel(str(money_value[int(i / 2)]), style.LabelFont1, money_style)]
            money_layout.addWidget(self.widgets["currency_amounts"][int(i/2)], i, 3)
            money_layout.addWidget(QTHelper.CreateSeperator(), i + 1, 3)
            # V SEPERATORS

            money_layout.addWidget(QTHelper.CreateVSeperator(), i, 0)
            money_layout.addWidget(QTHelper.CreateVSeperator(), i + 1, 0)
            money_layout.addWidget(QTHelper.CreateVSeperator(), i, 2)
            money_layout.addWidget(QTHelper.CreateVSeperator(), i + 1, 2)
            money_layout.addWidget(QTHelper.CreateVSeperator(), i, 4)
            money_layout.addWidget(QTHelper.CreateVSeperator(), i + 1, 4)
        ...
        encumberance_status_label0 = QTHelper.CreateLabel(
            "Weight: ",style.LabelFontSmallBold)
        
        
        self.encumberance_status_label1 = QTHelper.CreateLabel(
        ...
        sorting_layout.addWidget(QTHelper.CreateLabel("Sorting",style.LabelFont1))
    ...
    def make_stacked_layout(self):
        ...
        for i in range(num_of_pages):
            ...
            for k in range(num_per_column):
                if j < num_of_items:
                    current_page_left_layout.addWidget(QTHelper.ItemLabel(items[j]), k, 0)
                    j += 1

            for k in range(num_per_column):
                if j < num_of_items:
                    current_page_right_layout.addWidget(QTHelper.ItemLabel(items[j]), k, 0)
                    j += 1
...
class EncumberanceLabel(QFrame):
    ...
    def create_bar_widget(self):
        ...
        current_weight = self.encumberance.get_weight().getValue()
        ...
        for i in range(self.num_of_bars):
            if i == lvl0 and not past_bar[0]:
                current_level += 1
                past_bar[0] = True
                current_weight_marker = QTHelper.CreateLabel(str(set_points[0]) + " lbs-", weight_style)
            ...
            if i == lvl1 and not past_bar[1]:
                current_level += 1
                past_bar[1] = True
                current_weight_marker = QTHelper.CreateLabel(str(set_points[1]) + " lbs-", weight_style)
                policy = current_weight_marker.sizePolicy()
                policy.setVerticalPolicy(policy.Policy.Maximum)
                policy.setHorizontalPolicy(policy.Policy.Maximum)
                current_weight_marker.setSizePolicy(policy)
            ...
            if i == lvl2 and not past_bar[2]:
                current_level += 1
                past_bar[2] = True
                current_weight_marker = QTHelper.CreateLabel(str(set_points[2]) + " lbs-", weight_style)
                policy = current_weight_marker.sizePolicy()
                policy.setVerticalPolicy(policy.Policy.Maximum)
                policy.setHorizontalPolicy(policy.Policy.Maximum)
                current_weight_marker.setSizePolicy(policy)
            ...
            if i == current_weight_bar:
                current_weight_marker = QTHelper.CreateLabel("-" + str(current_weight) + "lbs", style.LabelFont2)
                policy = current_weight_marker.sizePolicy()
                policy.setVerticalPolicy(policy.Policy.Maximum)
                policy.setHorizontalPolicy(policy.Policy.Maximum)
                current_weight_marker.setSizePolicy(policy)
            ...
            bar = QTHelper.MakeColorWidget(color,opacity)
