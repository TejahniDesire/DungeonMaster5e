...
class ItemLabel(QFrame):
    ...
    def __init__(self, item: itemsDnD.Item, ATinventory: inventory.AttackInventory = None, update_func=None):
        super().__init__()
    ...
    def change_amount(self, operation: str):
        ...
        amount = pyHelper.ReferenceNumber(0, is_int=True)
        ...
        if operation == 'add':
            ...
            if self.ATinventory is None:
                function1 = partial(pyHelper.preform, [self.item.add_amount], [amount.getValue])
            else:
                print("RIGHT")
                function1 = partial(pyHelper.preform, [self.ATinventory.add_attack_item, self.update_func],
                                    [self.item])
            ...
            function1 = partial(pyHelper.preform, [self.item.safe_subtract], [amount.getValue])
            confirm_button = CreateGenButton(
                stylesheet=style.ConfirmEditButton,
                icon_url=imageURLS.CheckUrl,
                icon_size=QtCore.QSize(20, 20),
...
class ShopItemLabel(ItemLabel):
    def __init__(self, item: itemsDnD.Item, ATinventory: inventory.AttackInventory, update_func):
