...
class ItemLabel(QFrame):
    ...
    def change_amount(self, operation: str):
        ...
        amount = objectHelperTools.ReferenceNumber(0, is_int=True)
        ...
        if operation == 'add':
            ...
            if self.ATinventory is None:
                function1 = partial(objectHelperTools.preform, [self.item.add_amount], [amount.getValue])
            else:
                print("RIGHT")
                function1 = partial(objectHelperTools.preform, [self.ATinventory.add_attack_item, self.update_func],
                                    [self.item])
            ...
            function1 = partial(objectHelperTools.preform, [self.item.safe_subtract], [amount.getValue])
            confirm_button = CreateGenButton(
                stylesheet=style.ConfirmEditButton,
                icon_url=imageURLS.CheckUrl,
                icon_size=QtCore.QSize(20, 20),
